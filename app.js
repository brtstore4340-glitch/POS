...
async function handleImportData(file) {
    ...
    try {
        ...
        console.log(`📊 Import starting: ${jsonData.length} rows from Excel`);
        ...
        if (clearConfirmed) {
            ...
            const deleteCollection = async (colPath) => {
                ...
                console.log(`  Deleting ${snapshot.size} docs from ${colPath}`);
        ...
        console.log('⚙️ Preparing operations...');
        let operations = [];
        let validCount = 0;
        ...
        console.log(`✅ Prepared ${operations.length} operations from ${validCount} products`);
        ...
        console.log(`📦 Created ${batches.length} batches (${BATCH_SIZE} ops each)`);
        ...
        // Simple concurrency queue with error tracking
        const results = [];
        const executing = new Set();
        let errorCount = 0;
        ...
        for (let i = 0; i < batches.length; i++) {
            ...
            const p = batch.commit()
                .then(() => {
                    ...
                    console.log(`  ✅ Batch ${processedBatches}/${totalBatches} (${percent}%)`);
                    Swal.update({
                        html: `<b>${percent}%</b><br>Uploaded ${processedBatches} / ${totalBatches} batches`
                    });
                })
                .catch(err => {
                    executing.delete(p);
        ...
        console.log(`🎉 All batches completed! Errors: ${errorCount}`);
        ...
        try {
            ...
            console.log('📝 Import logged to system_imports');
